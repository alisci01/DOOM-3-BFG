...
idStr StripDeadCode( const idStr & in, const char * name ) {
	...
	while ( stack.Num() > 0 ) {
		...
		idLexer srcBlock( LEXFL_NOFATALERRORS );
		srcBlock.LoadMemory( blocks[i].postfix.c_str(), blocks[i].postfix.Length(), name );
		while ( srcBlock.ReadToken( &token ) ) {
